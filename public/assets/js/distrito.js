document.addEventListener("DOMContentLoaded", function () {
    ...
    if (agregarForm) {
      agregarForm.addEventListener("submit", function (event) {
        ...
        if (!/^[a-zA-ZáéíóúÁÉÍÓÚñÑ\s]+$/.test(distritoInput.value.trim())) {
          event.preventDefault();
          toastr.error(
            "El nombre del distrito solo debe contener letras y espacios."
          );
          return;
        }
        if (
          distritoInput.value.trim().length < 3 ||
          distritoInput.value.trim().length > 50
        ) {
          event.preventDefault();
          toastr.error(
            "El nombre del distrito debe tener entre 3 y 50 caracteres."
          );
          return;
        }
        if (!/^[A-ZÁÉÍÓÚÑ][a-záéíóúñ\s]+$/.test(distritoInput.value.trim())) {
          event.preventDefault();
          toastr.error(
            "El nombre del distrito debe comenzar con una letra mayúscula."
          );
          return;
        }
        if (distritoInput.value.trim() !== distritoInput.value) {
          event.preventDefault();
          toastr.error(
            "El nombre del distrito no debe tener espacios al inicio o al final."
          );
          return;
        }
        ...
        if (
          palabrasProhibidas.some((palabra) =>
            distritoInput.value.toLowerCase().includes(palabra)
          )
        ) {
          event.preventDefault();
          toastr.error(
            "El nombre del distrito contiene palabras no permitidas."
          );
          return;
        }
        event.preventDefault();
        verificarDistritoDuplicado(
  ...
  document
    .querySelectorAll('form[action*="/distritos/editar"]')
  ...
  document
    .querySelectorAll('form[action*="/distritos/eliminar"]')
    .forEach(function (form) {
      form.addEventListener("submit", function (event) {
        event.preventDefault();
        Swal.fire({
